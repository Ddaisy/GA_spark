...
object ga {
  ...
  def main(args: Array[String]) {
    ...
    var sigy = sc.hadoopFile("hdfs://10.131.247.181:9000/user/hadoop/test.txt", classOf[TextInputFormat], classOf[LongWritable], classOf[Text])
      .map(x => (x._1.toString.toInt, x._2.toString)).mapValues(transFormat)
    var dty = sc.textFile("hdfs://10.131.247.181:9000/user/hadoop/testa.txt").map(transFormat).collect().flatten

    val Dysum = preDysum.testPreData(sigy, dty)
    //Dysum.collect().foreach(println)

    var size = sc.parallelize(0 to 99)
    var initArray = size.map(x => (x, new Array[Double](10)))
    var populationArray = initArray.mapValues(x => initialPopulation.initialPopulation(x))
    var fit = populationArray.mapValues(x => fitness.fitnessFcn(x))


    populationArray.collect().foreach(_._2.foreach(println))
